...
export default function EditPropulsionType({ selectedRowData, edit, onClose }) {
  ...
  const uniqueNamesSet = new Set(propulsionTypes.map((item) => item.name));
  const uniqueNames = Array.from(uniqueNamesSet);

  const updateHandler = async (selectedRowData) => {
    if (selectedRowData) {
      try {
        const updatePropulsionType = await axios({
          url: `${API_URL}/propulsion_type/${selectedRowData.id}/`,
          method: "PUT",
          data: {
            ...propulsionType,
          },
        });
        if (updatePropulsionType.status == 200) {
  ...
  const getPropulsionTypes = async () => {
    try {
      ...
      if (res.status === 200) {
        // console.log("res", res);
        setPropulsionTypes(res?.data?.data);
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (edit && selectedRowData) {
      setPropulsionType(selectedRowData);
    }
    getPropulsionTypes();
                ...
                <div class="grid grid-cols-2 gap-4 mb-3">
                  ...
                  <div>
                    ...
                    <select className="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                      onChange={(e) => { setPropulsionType({ ...propulsionType, name: e.target.value, }); }}
                    >
                      {uniqueNames.map((name, index) => {                       
                        return (
                          <option key={index} value={name} selected={propulsionType.name === name}>
                            {name}
                          </option>
                        )
                      })}                      
