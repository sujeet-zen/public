...
export default function AddNewpropulsionType(props) {
  ...
  const submitHandler = async () => {
    if (
      ...
      try {
        const propulsionTypeRes = await axios({
                ...
                <div class="grid grid-cols-2 gap-4 mb-3">
                  ...
                  <div>  
                    ...
                    <select className="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                      ...
                      value={propulsionType?.type}
                    >
                      <option>Please Select</option>
                      <option value='Twin petrol'>Twin petrol</option>      
                      <option value='Single Diesel'>Single Diesel</option>                      
                    </select>
                  </div>
                </div>                                                           
              </form>
