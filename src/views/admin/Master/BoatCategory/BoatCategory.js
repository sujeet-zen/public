...
import AddNewBoatCategory from "./AddNewBoatCategory";
import EditBoatCategory from "./EditBoatCategory";
...
const BoatCategory = () => {
    ...
    if (rowId) {
      try {
        const deletePort = await axios({
          url: `${API_URL}/boat_category/${rowId}/`,
          method: "DELETE",
        });
        if (deletePort) {
          getBoatCategories();
  ...
  const data = useMemo(() => boatCategories, [boatCategories]);
  return (
    <>
      <Layout>
        <div className="sm:-mt-40 w-full">
          <div className="min-h-screen bg-white text-gray-900 shadow-lg rounded-3xl">
            <main className=" mx-auto px-4 sm:px-6 lg:px-8 pt-4">
              ...
              <div className="mt-6">
                <div className="flex flex-row">
                  <AddNewBoatCategory onClose={() => setShow(false)} show={show} />
                  <EditBoatCategory onClose={handleClose} edit={edit} selectedRowData={selectedRowData} />                                  
                </div>
