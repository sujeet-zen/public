...
import AddBoatEntity from "./AddBoatEntity";
// import { toast } from "react-toastify";
// import API from "../../../Redux/axios";

const BoatEntity = () => {
  const [boatEntityData, setBoatEntityData] = useState([]);
  ...
  const getTripsData = async () => {
    try {
      ...
      if (res.status === 200) {
        console.log("res", res);
        setBoatEntityData(res?.data?.data);
      }
    } catch (err) {
  ...
  const data = useMemo(() => boatEntityData, [boatEntityData]);
  return (
    <>
      <Layout>
        <div className="sm:-mt-40 w-full">
          <div className="min-h-screen bg-white text-gray-900 shadow-lg rounded">
            <main className=" mx-auto px-4 sm:px-6 lg:px-8 pt-4">
              ...
              <div className="mt-6">
                <div className="flex flex-row">
                  <AddBoatEntity onClose={() => setShow(false)} show={show} />

                  <div className="relative w-1/4  lg:max-w-sm mr-5">
                    <label>Boat Entity Type</label>
                    <select
                      id=""
                    ...
                    >
                      ...
                      <option value="LWS">Lower Deck Seat</option>
