...
import AddNewParcelType from "./AddNewParcelType";
import EditParcelType from "./EditParcelType";
// import '../../../../App.css'
...
const ParcelType = () => {
  ...
  const handleDelete = async (row) => {
    ...
    if (rowId) {
      try {
        const deleteParcel = await axios({
          url: `${API_URL}/parcel_type/${rowId}/`,
          method: "DELETE",
        });
        if (deleteParcel) {
          getParcelTypes();
  ...
  const data = useMemo(() => parcelTypes, [parcelTypes]);
  return (
    <>
      <Layout>
        <div className="sm:-mt-40 w-full">
          <div className="min-h-screen bg-white text-gray-900 shadow-lg rounded-3xl">
            <main className=" mx-auto px-4 sm:px-6 lg:px-8 pt-4">
              ...
              <div className="mt-6">
                <div className="flex flex-row">
                  <AddNewParcelType onClose={() => setShow(false)} show={show} />
                  <EditParcelType onClose={handleClose} edit={edit} selectedRowData={selectedRowData} />                                  
                </div>
