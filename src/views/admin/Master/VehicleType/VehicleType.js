...
import AddNewVehicleType from "./AddNewVehicleType";
...
const VehicleType = () => {
  ...
  const handleDelete = async (row) => {
    ...
    if (rowId) {
      try {
        const deleteVehicle = await axios({
          url: `${API_URL}/vehicle_master/${rowId}/`,
          method: "DELETE",
        });
        if (deleteVehicle) {
          getVehicleTypes();
  ...
  const data = useMemo(() => vehicleTypes, [vehicleTypes]);
  return (
    <>
      <Layout>
        <div className="sm:-mt-40 w-full">
          <div className="min-h-screen bg-white text-gray-900 shadow-lg rounded-3xl">
            <main className=" mx-auto px-4 sm:px-6 lg:px-8 pt-4">
              ...
              <div className="mt-6">
                <div className="flex flex-row">
                  <AddNewVehicleType onClose={() => setShow(false)} show={show} />
                  <EditVehicleType onClose={handleClose} edit={edit} selectedRowData={selectedRowData} />                                  
                </div>
