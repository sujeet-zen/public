...
export default function EditBoatEntity({ selectedRowData, edit, onClose}) {    
  ...
  const uniqueNamesSet = new Set(boatEntities.map((item) => item.name));
  ...
  const updateHandler = async (selectedRowData) => {               
    ...
    try {
      const updatePort = await axios({
        url: `${API_URL}/boat_entity/${selectedRowData.id}/`,
        method: "PUT",
        data: {
          ...boatEntity,
        },
                  ...
                  <div>
                    ...
                    >                      
                       {uniqueNames.map((name, index) => {                                                                                                       
                        return (
                          <option key={index} value={name} selected={boatEntity.name === name}>                            
                          {name}                                            
                          </option>                          
                        )
                      })}                                                       
