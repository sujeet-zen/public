...
export default function EditPetType({ selectedRowData, edit, onClose }) {
  ...
  const uniqueNamesSet = new Set(petTypes.map((item) => item));
    ...
    if (selectedRowData) {
      try {
        const updatePetType = await axios({
          url: `${API_URL}/pet_master/${selectedRowData.id}/`,
          method: "PUT",
          data: {
            ...petType,
          },
                ...
                <div class="grid grid-cols-2 gap-4 mb-3">
                  ...
                  <div>
                    ...
                    <select className="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                      onChange={(e) => { setPetType({ ...petType, name: e.target.value, }); }}
                    >
                      {uniqueNames.map((name, index) => {
                       const {type} = name;
                        return (
                          <option key={index} value={type} selected={petType.type === type}>
                            {name.type}
                          </option>
                        )
                      })}                      
                ...
                <div class="grid grid-cols-2 gap-4 mb-3">
                  ...
                  <div>
                    ...
                    >
                       ...
                       const {weight} = name;
                        return (
                          <option key={index} value={weight} selected={petType.weight === weight}>
                            {name.weight}
                          </option>
                        )
                      })}                      
